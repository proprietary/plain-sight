...
class video_output_t {
  public:
    virtual auto get_io_context() const noexcept -> AVIOContext * const = 0;
    virtual ~video_output_t() noexcept = 0;
};

class in_memory_video_output_t : public video_output_t {
  public:
    explicit in_memory_video_output_t();
    auto get_io_context() const noexcept -> AVIOContext * const override;
  ...
  private:
    ...
    std::uint8_t* buffer_;
    std::vector<std::uint8_t> buffer_contents_;
    ...
    static int write_packet(void *opaque, std::uint8_t *buf, int buf_size) noexcept;
};
...
class encoder_t {
  public:
    class builder_t {
      public:
        auto set_video_output(std::shared_ptr<video_output_t> video_output) noexcept -> builder_t&;
  ...
  private:
    explicit encoder_t(std::shared_ptr<video_output_t> video_output,
                       std::shared_ptr<std::vector<qrcodegen::QrCode>> qr_codes,
                       std::string video_format, const size_t scale, const size_t border_size, const int fps = 20) noexcept
        : video_output_{std::move(video_output)}, qr_codes_{qr_codes},
          video_format_{video_format}, scale_{scale}, border_size_{border_size}, fps_{fps} {}
